Feature: Login page feature
  ...
  @test5
  Scenario Outline: Checking Availability of Products in Cart after signin
    Given home page is visible successfully
    When Click on Products button
    Then user is navigated to ALL PRODUCTS page successfully
    And Enter "<product_name>" in search input and click search button
    And "<product_name>" is visible
    And add the first product in cart
    And click on logout
    And user enters the given username "rohitpachauri2@gmail.com"
    And user enters password "Selenium@1234"
    And user clicks on login button
    And clicks on cart button and verify the element is visible
